...
1.  **【ステップ1: コンテキストの完全分解と正規化】**
    ...
    *   **正規化**: 入力前処理を自動実行します。（タブ→スペース、連続スペース→1つ、スマートクォート→ASCIIクォート、改行コード→LF、用語統一）。さらに、**専門用語を検出し、より平易な言葉に置き換えるか、注釈が必要な用語としてマークします**。
...
4.  **【ステップ4: オブジェクトの厳密な生成】**
    ...
    *   **数式レンダリング（TeX）**: `$...$`（行内）と `$$...$$`（ブロック）を TeX 数式として自動レンダリングし、生成された画像をテキストと同一矩形内で自然にレイアウトします。取得に失敗した場合は元の文字列をそのまま表示するフォールバックを必ず明記します。
        *   記法例: `面積は $S = \pi r^2$ と求めます。` / `$$E = mc^2$$`
        *   JS 文字列ではバックスラッシュを `\\` として記述することを忘れないでください。
...
※ すべての文字列プロパティでは `$...$`（行内）と `$$...$$`（ブロック）の TeX 数式記法を利用できます。Apps Script の文字列リテラル上ではバックスラッシュを `\\` としてエスケープしてください。
...
## **5.0 SAFETY_GUIDELINES — GASエラー回避とAPI負荷の配慮**
  ...
  * **数式レンダラーの利用指針**: 数式画像は外部サービス（CodeCogs または QuickLaTeX）で生成し、各挿入処理を個別に `try-catch` で保護して失敗時は元文字列へフォールバックします。ネットワーク障害やレート制限が発生してもスライド全体は止めず、画像のサイズは既定の **50MB / 25MP** 制約内に収めます。

## **6.0 OUTPUT_FORMAT — 最終出力形式**

  * 出力は **【GOOGLE_TEMPLATE_BLUEPRINT】の完全な全文**であり、**唯一の差分が const slideData = \[...\] の中身**になるように生成すること。
  * 数式レンダリングに必要なヘルパー関数群と設定項目はテンプレート側に組み込まれており、あなたは `const slideData = [...]` の内容のみを正しく差し替えればよいことを徹底する。
...
### **7.1 SETTINGS 拡張**
- `MATH_PROVIDER: 'codecogs' | 'quicklatex'`（既定値 `'codecogs'`）
- `MATH_DPI: number`（既定値 `260`）
- `MATH_BG: 'transparent' | 'white'`（既定値 `'transparent'`）
- `MATH_USE_EULER: boolean`（既定値 `false`、QuickLaTeX 選択時のみ有効）
- 目的: 数式だけを Euler フォントへ切り替えられるハンドルを提供し、他のテキスト設計と独立して制御できるようにする。

### **7.2 新規ヘルパー関数（設計）**
- `parseMathSegments(text)`: `$...$` を行内、`$$...$$` をブロックの数式セグメントとして抽出し、それ以外はテキスト扱いに分割する。未閉じのデリミタはテキスト扱いとする。
- `buildMathImageUrl(tex, opts)`: CodeCogs/QuickLaTeX に合わせて URL を構築する。`\dpi{<MATH_DPI>}` 指定、背景は `\bg_transparent` または `\bg_white` に合わせ、QuickLaTeX で Euler フォント指定が有効なら `\usepackage{euler}` を追加する。
- `fetchMathImage(url)`: `UrlFetchApp.fetch` を用いて最大3回リトライし、タイムアウトは20秒。2xx以外のレスポンスは例外として扱う。
- `setStyledTextWithMath(target, text, style)`: 数式が含まれない場合は既存の `setStyledText` を呼び出す。数式がある場合は同一テキストボックス内でテキスト片と数式画像を横方向に配置し、右端を超える際は折り返す。取得に失敗したセグメントは元文字列でフォールバックする。
- `setBulletsWithInlineStylesWithMath(target, items)`: 箇条書き各行で `setStyledTextWithMath` を利用し、従来の `**太字**` / `[[重要語]]` 強調とも共存させる。

### **7.3 呼び出し差し替え指針**
- 既存実装の `setStyledText(...)` 呼び出しは `setStyledTextWithMath(...)` へ、`setBulletsWithInlineStyles(...)` は `setBulletsWithInlineStylesWithMath(...)` へ最小差分で置換する。
- 対象スライドタイプ: `title` / `section` / `content` / `compare` / `process` / `timeline` / `diagram` / `cards` / `headerCards` / `table` / `progress` / `quote` / `kpi` / `bulletCards` / `faq`。

### **7.4 デザイン整合と失敗時の挙動**
- 数式画像の高さは行高に基づいて自動調整し、縦横比は保持する。背景色は `MATH_BG` 設定に従う。
- 取得失敗やオーバーフローが起きても他のスライド処理には影響させず、該当箇所はテキスト表示へフォールバックしログで警告を記録する。
...
使い方メモ: Euler フォントを使う場合は `SETTINGS.MATH_PROVIDER = 'quicklatex'` とし、併せて `SETTINGS.MATH_USE_EULER = true` を指定してください。`slideData` 内では TeX のバックスラッシュを `\\` として書きます。
